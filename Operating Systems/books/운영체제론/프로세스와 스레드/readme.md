...
인터럽트를 발생시키는 장치들은 프로세서에ㅅ는 외부 개체들이다. 이러한 인터럽트들은 실행되는 명령어들과 무관하게 발생하므로 실행중인 프로세스와 비동기적이다. 한편 시스템 호출과 같은 소프트웨어 생성 인터럽트들은 소프트웨어의 특정 명령어 때문에 인터럽트가 생성되므로 실행중인 프로세스와동기적이다. 